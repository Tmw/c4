...
const Cell: React.FC<CellProps> = (props: CellProps) => {
  return (
    <div className={`cell cell-state-${props.cell}`}></div>
  )
}
...
const Column: React.FC<ColumnProps> = (props: ColumnProps) => {
  return (
    <div className="column" onClick={() => props.onClick(props.columnId)}>
      {props.column.map((cell: State.Cell, idx: number) => <Cell cell={cell} key={`cell-${idx}`} />)}
    </div>
  )
}
...
function useGameState(){
  ...
  const playColumn = (columnId: number) => {
    // find open spot
    const openIndex = state.board[columnId]
      .findLastIndex((cell: State.Cell) => cell === 'open')
...
export default function App() {
  ...
  const clickColumn = useCallback((idx: number) => {
    playColumn(idx);
  ...
  return (
    <div className="App">
      ...
      <div className={`board current-player-${currentPlayer}`}>
        {board.map((column: State.Column, idx: number) => <Column column={ column } onClick={clickColumn} columnId={idx} key={`column-${idx}`}/>)}
      </div>
    </div>
  )
}